defmodule EasyBreezy.Layouts.TwoColumnSlide do
  ...
  import Breeze.Blocks, only: [panel: 1]
  ...
  def two_column_slide(assigns) do
    ...
    visible_right_lines =
      right_lines
      |> Enum.flat_map(&wrap_text_line(&1, right_width))
    ...
    left_content_height =
      max(panel_height - if(image_mode?, do: 2, else: left_header_height + 2), 1)

    right_content_height =
      max(panel_height - if(image_mode?, do: 2, else: right_header_height + 2), 1)
    ...
    <box class="width-full height-full">
      ...
      <box :if={@image_mode?} style={@panel_style} class="grid grid-cols-2 width-full">
        <.panel
          id="slide-left-panel"
          class="width-full height-full"
          focus_within={false}
          hide_focus_indicator
        >
          <:title :if={@left_title}>{@left_title}</:title>
          ...
          <box
            ...
            class="width-full bg-panel overflow-hidden"
          >
          </box>
        </.panel>
        <.panel
          id="slide-right-panel"
          class="width-full height-full"
          focus_within={false}
          hide_focus_indicator
        >
          <:title :if={@right_title}>{@right_title}</:title>
          <box :if={@right_notice && not @show_right_lines?} class="text-muted padding-1">
            {@right_notice}
          </box>
          <box
            :for={{class, line} <- @visible_right_lines}
            :if={@right_mode == :text && @show_right_lines?}
            class={right_line_class(class, true)}
          >
            {line}
          </box>
          <.mermaid
      ...
      >
        ...
        <box style={@panel_style} class={@right_panel_class}>
          ...
          <box
            :for={{class, line} <- @visible_right_lines}
            :if={@right_mode == :text && @show_right_lines?}
            class={right_line_class(class, false)}
          >
            {line}
          </box>
          <.mermaid
  ...
  defp normalize_line("## " <> heading), do: {"bold text-secondary", String.trim(heading)}
  defp normalize_line(line), do: {"", line}

  defp wrap_text_line(line, width) do
    {class, text} = normalize_line(line)
    Enum.map(wrap_code_line(text, width), &{class, &1})
  end

  defp right_line_class("", true),
    do: "text-accent padding-left-1 padding-right-1"

  defp right_line_class(class, true),
    do: class <> " padding-left-1 padding-right-1"

  defp right_line_class("", false), do: "text-accent"
  defp right_line_class(class, false), do: class
